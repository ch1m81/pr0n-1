define([
    ...
    pr0nApp.module('Entities', function(Entities, pr0nApp, Backbone, Marionette, $, _) {
        
        Entities.ArticleModel = Backbone.Model.extend();
        // Put the methods in the prototype because they are going to be the same always
        _.extend(Entities.ArticleModel.prototype, {
        ...
        var API = {
            getExampleEntities: function() {
                var collection = new Entities.ArticleCollection();
                var defer = $.Deferred();
                collection.fetch({
            ...
            getExampleEntity: function(exampleName) {
                var model = new Entities.ArticleModel({
                    name: exampleName
                });
                var defer = $.Deferred();
                model.fetch({
        ...
        //Get all models
        pr0nApp.reqres.setHandler('example:entities', function() {
            return API.getExampleEntities();
        });
        //Get a specific model by id
        pr0nApp.reqres.setHandler('example:entity', function(id) {
            return API.getExampleEntity(id);
        });
        //Save a model to the server
        pr0nApp.reqres.setHandler('example:save', function(newModel, data) {
            return API.saveExampleEntity(newModel, data);
        });
        //Edit a model on the server
        pr0nApp.reqres.setHandler('example:edit', function(model, data) {
            return API.editExampleEntity(model, data);
        });
        //Delete a model on the server
        pr0nApp.reqres.setHandler('example:delete', function(model) {
